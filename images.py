...
def banner_create(data: bannerData) -> None | bytes:
    ...
    if data.user_descriminator != None and data.user_descriminator != "" and data.user_descriminator != "0":
    ...
    if data.banner_type == bannerType.JOIN or data.banner_type == bannerType.LEAVE:
        ...
        draw.text(((padding * 3) + profile_image.size[0], int(padding * 2 + int(padding / 2 * 2) + font_large.size)), "Current level:", "WHITE", font_small) #type: ignore | error is with .size, it will exist.
        draw.text(((padding * 13) + profile_image.size[0], int(padding * 2 + int(padding / 2 * 2) + font_large.size)), "@Apple", "YELLOW", font_small) #type: ignore | error is with .size, it will exist.
        draw.text(((padding * 3) + profile_image.size[0], int(padding * 2 + int(padding / 2 * 3) + font_large.size + font_small.size)), "Next level:", "WHITE", font_small)
        draw.text(((padding * 13) + profile_image.size[0], int(padding * 2 + int(padding / 2 * 3) + font_large.size + font_small.size)), "@Banana", "MAGENTA", font_small) #type: ignore | error is with .size, it will exist.

        start_x_y = ((padding * 3) + profile_image.size[0], int(padding * 2 + int(padding / 2 * 3) + font_large.size + (font_small.size * 2) + (padding * 2)))
        ...
        end_pos = ((random_percentage / 1000) * (end_x_y[0] - start_x_y[0])) + start_x_y[0]
